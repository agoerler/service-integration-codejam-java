...
annotate IncidentsService.Incidents with @(UI: {
  ...
  FieldGroup #HeaderGeneralInformation: {Data: [
    ...
    {Value: service.type},
    {
      ...
      Target: 'service/worker/@Communication.Contact'
    }
  ]},
...
annotate IncidentsService.Conversation with @(UI: {
  ...
  FieldGroup #MessageDetails: {Data: [{Value: message}]}
});
...
annotate IncidentsService.Appointments with @(UI: {LineItem: [
  {Value: start_date},
  {Value: end_date},
  {
    Value: worker.name,
